...
def read_jpeg(file_name: str):
    ...
    with open(file_name, "rb") as f:
        ...
        if SOI_marker != b'\xFF\xD8':
            ...
            exit()
        logger.debug(f"Correct start byte for SOI. Proceeding.")
        
        APP0_marker = f.read(2)
        if APP0_marker != b'\xFF\xE0':
            logger.error("incorrect APP0 marker")
            exit()
        ...
        if (JFIF_identifier != "JFIF\0"):
            logger.error(f"JFIF Identifier \"{JFIF_identifier}\" != \"JFIF\\0\"")
            exit()
        if (JFIF_major_ver != 1):
            logger.error(f"JFIF Major Version {JFIF_major_ver} != 1")
            exit()
        if not 0 <= JFIF_minor_ver <= 2:
            logger.error(f"JFIF Minor Version {JFIF_minor_ver} not in range 0-2")
            exit()
        if not 0 <= units <= 2:
            logger.error(f"Invalid units specifier {units}")
            exit()
